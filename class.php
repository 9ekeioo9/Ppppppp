...
public function Gmail($UseName){
    ...
    public function Yahoo($UseName)
    {
        ...
        curl_setopt($c, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36");
        curl_setopt($c, CURLOPT_REFERER, 'https://www.google.com');
        curl_setopt($c, CURLOPT_ENCODING, 'gzip, deflate, br');
        curl_setopt($c, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($c, CURLOPT_HEADER, true);
        curl_setopt($c, CURLOPT_COOKIEJAR, "yahoo.txt");
        curl_setopt($c, CURLOPT_COOKIEFILE, "yahoo.txt");
        ...
        $data = "acrumb=$acrumb&sessionIndex=$sesindex&username=" . urlencode($UseName) . "&passwd=&signin=Next";
        $c = curl_init("https://login.yahoo.com/");
        curl_setopt($c, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($c, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36");
        ...
        $b = @curl_exec($c);
        if(isset($b)){
        if (strstr($b, 'messages.ERROR_INVALID_USERNAME'))
        {
            return true;
        }
        else

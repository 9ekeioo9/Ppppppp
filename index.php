...
global $IGMail;
        $mail = strtolower($mail);
            if(mb_substr($mail, -10) === '@gmail.com' and $e == "gmail.com"){
            return $IGMail->verifyEmail($mail);
            } elseif(preg_match('/(live|hotmail|outlook)\.(.*)/', $mail) and $mail[-1] !=="k" and $mail[-1] !=="i" and $mail[-1] !=="t"){
                return checkHotmail($mail);
            } elseif(strpos($mail, 'yahoo.com') and $e == "yahoo.com" and !strstr($ei,".")){
                return checkYahoo($mail);
            } elseif(strpos($mail, 'aol.com')){
                return checkAol($mail);
            } elseif(preg_match('/(mail|bk|yandex|inbox|list)\.(ru)/i', $mail) and !strstr($mail,"+")){
                return checkRU($mail);
            } else {
                return false;
            }
...
if(isset($user->public_email)){
  ...
  $ret = false;
    ...
    if($search->message == 'Please wait a few minutes before you try again.' or $search->message == 'challenge_required' or $search->message == 'feedback_required' or $search->message == 'login_required' ){
        $ret = 'checkpoint';
    }
     else {
...
function checkRU($mail){
  $html = file_get_contents("https://account.mail.ru/api/v1/user/exists?email=$mail");
  if(preg_match('/"exists":false,/', $html)){
    return true;
  }else{ 
    return false;
  } 
...
function checkYahoo($user){
    $c = curl_init("https://login.yahoo.com/"); 
    curl_setopt($c, CURLOPT_FOLLOWLOCATION, true); 
    curl_setopt($c, CURLOPT_USERAGENT, "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36"); 
    ...
    $b = curl_exec($c); 
    if(strstr($b,'messages.ERROR_INVALID_USERNAME') or strstr($b,'messages.INVALID_USERNAME') ){
      return true;
    }else{ 
      return false; 
    } 
...
function checkHotmail($mail){
  $html = file_get_contents("https://signup.live.com/signup?username=$mail");
  if(preg_match('/"isAvailable":1}/', $html)){
    return true;
  }else{ 
    return false;
  } 
...
class ig {
	...
	public function __construct($settings){
		...
		$this->account['useragent'] = 'Instagram 123.1.0.26.115 (iPhone10,3; iOS 13_3; en_US; en-US; scale=3.00; 1125x2436; 190542906)';
	...
	private function request($path,$account = 0,$post = 0,$datas = 0,$returnHeaders = 0){
	  ...
	  if($account == 0){
	  	curl_setopt($ch, CURLOPT_HTTPHEADER, array(
		     ...
		     'user-agent: Instagram 123.1.0.26.115 (iPhone10,3; iOS 13_3; en_US; en-US; scale=3.00; 1125x2436; 190542906)',
		     'host: i.instagram.com',
		     'X-CSRFToken: missing',
		     'X-Instagram-AJAX: 1',
